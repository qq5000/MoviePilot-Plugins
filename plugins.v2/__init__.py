...
class P115StrmHelper(_PluginBase):
    ...
    def get_command() -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {
                "cmd": "/p115_convert_strm",
                "event": EventType.PluginAction,
                "desc": "批量修正STRM文件格式",
                "category": "",
                "data": {"action": "p115_convert_strm"},
            },
        ]
...
"""
    ...
    @eventmanager.register(EventType.PluginAction)
    def p115_convert_strm(self, event: Event):
        """
        批量修正STRM文件格式
        """
        if not event:
            return
        event_data = event.event_data
        if not event_data or event_data.get("action") != "p115_convert_strm":
            return
        ...
        text = f"""
📂 处理路径：{len(local_paths)} 个
✅ 成功转换：{total_converted} 个文件
❌ 转换失败：{total_errors} 个文件
"""
        
        self.post_message(
            channel=event.event_data.get("channel"),
            userid=event.event_data.get("user"),
            title="✅【115网盘】STRM格式批量修正完成",
            text=text,
        )
